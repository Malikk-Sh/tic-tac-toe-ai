//Get the board, record, text to traslate and select html elements
const gridSpaces = document.querySelectorAll('[data-spaces]');
const recordText = document.querySelector('[data-record]');
const difficultySelect = document.querySelector('[data-diff-select]')
const languageSelect = document.querySelector('[data-lang-select]')
const translateText = document.querySelectorAll('[data-translate]')
...
let languages = [
    ["Easy", "Medium", "Hard", "Unbeatable", "Two players"],
    ["Легкий", "Средний", "Сложный", "Непобедимый", "Два игрока"]
]

function updateLanguage() {
    let language = Number(languageSelect.value);
    for (let i = 0; i < languages[language].length; i++) {
        translateText[i].innerHTML = languages[language][i];   
    }
}
updateLanguage()